...
argparser = argparse.ArgumentParser(
    ...
    + "\n\n"
    + twf(
        "- Single-file mode (with options -o,-c and/or -S): Compile and/or assemble and/or link a single program."
    ),
    epilog="examples:\n\n"
...
argparser.add_argument(
    "-S",
    action="store_true",
    help="single-file mode: compile from C to assembly, but do not assemble",
)
argparser.add_argument(
    "-c",
    action="store_true",
    help="single-file mode: compile/assemble to machine code, but do not link",
)
argparser.add_argument(
    "-o",
    "--output",
    metavar="OUTPUTNAME",
    help="single-file mode: write output to that file",
)
...
if args.S or args.c or args.output:
    ...
    if args.S:  # produce assembly
        ...
        ifccstatus = run_command(
            f"{pld_base_dir}/compiler/ifcc {inputfilename} > {args.output}"
        )
        if ifccstatus:  # let's show error messages on screen
            exit(
                run_command(
                    f"{pld_base_dir}/compiler/ifcc {inputfilename}", toscreen=True
                )
            )
        else:
            exit(0)
        ...
        ifccstatus = run_command(
            f"{pld_base_dir}/compiler/ifcc {inputfilename} > {asmname}"
        )
        if ifccstatus:  # let's show error messages on screen
            exit(
                run_command(
...
for jobname in jobs:
    ...
    ## IFCC compiler
    ifccstatus = run_command(
        f"{pld_base_dir}/compiler/ifcc input.c > asm-ifcc.s", "ifcc-compile.txt"
    )
