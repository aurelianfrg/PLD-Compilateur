...
string CFG::new_BB_name() { return "bloc_" + to_string(nextBBnumber); }
...
void CFG::add_to_symbol_table(Symbol s) { symbolsTable.add(s); }
Symbol &CFG::access_symbol(string name) { return symbolsTable.access(name); }
...
void IRInstr::gen_asm_eq(ostream &os) {
  ...
  os << "    movzbl  %al, %eax"
     << endl; // move with conversion from byte to int
...
void IRInstr::gen_asm_div(ostream &os) {
  ...
  os << "    movl    " << "%eax" << ", " << destAddress
     << endl; // quotient is in %eax
}
...
void IRInstr::gen_asm_mod(ostream &os) {
  ...
  os << "    cltd" << endl; // Extend %eax into %edx:%eax for division
  os << "    idivl    " << v2Address << endl;
  os << "    movl    " << "%edx" << ", " << destAddress
     << endl; // Remainder is in %edx
}
