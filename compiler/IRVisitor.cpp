...
std::any IRVisitor::visitExpr_add_sub(ifccParser::Expr_add_subContext *ctx) {
    ...
    if (op == "+") {
        cfg->current_bb->add_IRInstr(IRInstr::add, Type::INT,
                                     {tempVar.getName(), tempVarName1, tempVarName2});
    }

    else if (op == "-") {
        cfg->current_bb->add_IRInstr(IRInstr::sub, Type::INT,
                                     {tempVar.getName(), tempVarName1, tempVarName2});
    }
...
std::any IRVisitor::visitExpr_mult_div_mod(ifccParser::Expr_mult_div_modContext *ctx) {
    ...
    if (op == "*") {
        cfg->current_bb->add_IRInstr(IRInstr::mul, Type::INT,
                                     {tempVar.getName(), tempVarName1, tempVarName2});
    } else if (op == "/") {
        cfg->current_bb->add_IRInstr(IRInstr::div, Type::INT,
                                     {tempVar.getName(), tempVarName1, tempVarName2});
    } else if (op == "%") {
        cfg->current_bb->add_IRInstr(IRInstr::mod, Type::INT,
                                     {tempVar.getName(), tempVarName1, tempVarName2});
    }
...
std::any IRVisitor::visitExpr_comp(ifccParser::Expr_compContext *ctx) {
    ...
    if (op == string("<")) {
        cfg->current_bb->add_IRInstr(IRInstr::cmp_lt, Type::INT,
                                     {resultTempVar.getName(), expr1Address, expr2Address});
    } else if (op == string("<=")) {
        cfg->current_bb->add_IRInstr(IRInstr::cmp_le, Type::INT,
                                     {resultTempVar.getName(), expr1Address, expr2Address});
    } else if (op == string(">")) {
...
std::any IRVisitor::visitExpr_eq_diff(ifccParser::Expr_eq_diffContext *ctx) {
    ...
    if (op == string("==")) {
        cfg->current_bb->add_IRInstr(IRInstr::cmp_eq, Type::INT,
                                     {resultTempVar.getName(), expr1Address, expr2Address});
    } else {
        cfg->current_bb->add_IRInstr(IRInstr::cmp_diff, Type::INT,
                                     {resultTempVar.getName(), expr1Address, expr2Address});
    }
...
std::any IRVisitor::visitExpr_and(ifccParser::Expr_andContext *context) {
    ...
    cfg->current_bb->add_IRInstr(IRInstr::bit_and, Type::INT,
                                 {resultTempVar.getName(), expr1Address, expr2Address});
    return resultTempVar.getName();
}
...
std::any IRVisitor::visitExpr_xor(ifccParser::Expr_xorContext *context) {
    ...
    cfg->current_bb->add_IRInstr(IRInstr::bit_xor, Type::INT,
                                 {resultTempVar.getName(), expr1Address, expr2Address});
    return resultTempVar.getName();
}
...
std::any IRVisitor::visitExpr_or(ifccParser::Expr_orContext *context) {
    ...
    cfg->current_bb->add_IRInstr(IRInstr::bit_or, Type::INT,
                                 {resultTempVar.getName(), expr1Address, expr2Address});
    return resultTempVar.getName();
}
...
std::any IRVisitor::visitExpr_log_and(ifccParser::Expr_log_andContext *ctx) {
    ...
    cfg->current_bb->add_IRInstr(IRInstr::copy, Type::INT, {tempVar.getName(), leftVarName});
    ...
    cfg->current_bb->add_IRInstr(IRInstr::copy, Type::INT, {tempVar.getName(), rightVarName});
